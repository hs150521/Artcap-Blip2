"""Adapter utilities for EfficientNet-B3 visual features."""
...
_DEFAULT_CHECKPOINT = Path("/home/linux/artcap-blip2-4/runs/efficientnet-28/best.pt")
...
class EfficientNetAdapter(nn.Module):
    ...
    def __init__(
        ...
        if not ckpt_path.exists():
            raise FileNotFoundError(
                ...
                "Please provide a valid path in config.paths.efficientnet_checkpoint."
            )
    ...
    def _load_backbone(ckpt_path: Path) -> nn.Module:
        ...
        # The checkpoint contains pathlib.PosixPath objects which are not allowed with weights_only=True
        ...
        # Create a custom EfficientNet model with the correct number of classes
        # Based on the checkpoint, it has 28 classes instead of the standard 1000
        model = efficientnet_b3(weights=None, num_classes=28)
        
        # Try to load state dict with strict=False to handle class mismatch
        missing, unexpected = model.load_state_dict(state, strict=False)
        if missing:
            print(f"[EfficientNetAdapter] Missing keys when loading checkpoint ({len(missing)}): {missing[:10]}")
        if unexpected:
            print(f"[EfficientNetAdapter] Unexpected keys when loading checkpoint ({len(unexpected)}): {unexpected[:10]}")
