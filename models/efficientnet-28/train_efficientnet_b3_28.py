...
class COCODataset(Dataset):
    ...
    def __init__(self, coco_root: Path, transform=None):
        ...
        self.targets = [27] * len(self.samples)
...
class CombinedWikiArtCOCODataset(Dataset):
    ...
    def _build_targets(self):
        targets = []
        wikiart_targets = getattr(self.wikiart_dataset, "targets", None)
        if wikiart_targets is not None:
            targets.extend(int(wikiart_targets[idx]) for idx in self.wikiart_indices)
        else:
            targets.extend(int(self.wikiart_dataset[idx][1]) for idx in self.wikiart_indices)

        coco_targets = getattr(self.coco_dataset, "targets", None)
        if coco_targets is not None:
            targets.extend(int(coco_targets[idx]) for idx in self.coco_indices)
        else:
            targets.extend(int(self.coco_dataset[idx][1]) for idx in self.coco_indices)
        return targets


def parse_args():
    parser = argparse.ArgumentParser(description="EfficientNet-B3 WikiArt+COCO trainer (28 classes)")
    parser.add_argument("--wikiart-root", type=Path, default=Path("/data/artcap-blip2-4/datasets/wikiart"),
                        help="Root folder containing 27 WikiArt style subfolders.")
    parser.add_argument("--coco-root", type=Path, default=Path("/data/artcap-blip2-4/datasets/coco"),
    ...
    parser.add_argument("--no-balance-sampler", action="store_true",
                        help="Disable per-class balanced sampling. Enabled by default.")
...
def build_class_balanced_sampler(targets, num_classes):
    if not targets:
        return None
    target_tensor = torch.tensor(targets, dtype=torch.long)
    class_counts = torch.bincount(target_tensor, minlength=num_classes)
    nonzero_mask = class_counts > 0
    if not torch.all(nonzero_mask):
        missing = [str(cls) for cls, present in enumerate(nonzero_mask.tolist()) if not present]
        print(f"Warning: Unable to balance classes with no samples: {', '.join(missing)}")
...
def main():
    ...
    if not args.no_balance_sampler:
        train_sampler = build_class_balanced_sampler(train_dataset.targets, num_classes)
        if train_sampler is not None:
            print("Using class-balanced WeightedRandomSampler for training.")
        else:
            print("Balanced sampler requested but no targets found; falling back to random shuffle.")
    else:
        print("Balanced sampler disabled via flag; using plain random shuffling.")
