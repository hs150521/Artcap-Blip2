"""
Modified Q-Former with FiLM-style KV modulation.

This module adapts the BLIP-2 Q-Former so that cross-attention layers can apply
FiLM modulation to their keys and values based on EfficientNet-derived style
controllers.
"""
...
from .kv_modulation import KVModulation


class BertSelfAttentionKVModulated(nn.Module):
    """
    BertSelfAttention variant that supports FiLM modulation of cross-attention
    keys and values using controller tokens derived from EfficientNet features.
    """
    ...
    def __init__(
        ...
        self.controller_tokens: Optional[torch.Tensor] = None
        if is_cross_attention:
            kv_cfg = kv_config or {}
            controller_dim = kv_cfg.get("controller_dim", config.hidden_size)
            self.kv_modulation = KVModulation(
                hidden_size=self.all_head_size,
                num_heads=self.num_attention_heads,
                controller_dim=controller_dim,
                hidden_dim=kv_cfg.get("hidden_dim"),
                init_scale=kv_cfg.get("init_scale", 0.01),
                use_layer_norm=kv_cfg.get("use_layer_norm", True),
            )
        else:
            self.kv_modulation = None

    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens shared across cross-attention layers."""
    ...
    def forward(
        ...
        if is_cross_attention:
            ...
            if self.kv_modulation is not None and self.controller_tokens is not None:
                key_layer, value_layer = self.kv_modulation(
                    key_layer, value_layer, self.controller_tokens
                )
...
class BertAttentionKVModulated(nn.Module):
    def __init__(self, config, is_cross_attention=False, kv_config: Optional[Dict] = None):
        super().__init__()
        self.self = BertSelfAttentionKVModulated(
            config,
    ...
    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens for cross-attention."""
        self.self.set_controller_state(controller_tokens)
...
class BertLayerKVModulated(nn.Module):
    def __init__(self, config, layer_num, kv_config: Optional[Dict] = None):
        super().__init__()
        self.config = config
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttentionKVModulated(config, kv_config=kv_config)
    ...
    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens for cross-attention in this layer."""
        if self.has_cross_attention:
            self.crossattention.set_controller_state(controller_tokens)
...
class BertEncoderKVModulated(nn.Module):
    def __init__(self, config, kv_config: Optional[Dict] = None):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList(
            [BertLayerKVModulated(config, i, kv_config=kv_config) for i in range(config.num_hidden_layers)]
        )

    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens for all cross-attention layers."""
        for layer in self.layer:
            layer.set_controller_state(controller_tokens)
...
class BertModelKVModulated(BertPreTrainedModel):
    ...
    def __init__(
        ...
        self.encoder = BertEncoderKVModulated(config, kv_config=kv_config)
    ...
    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens for all cross-attention layers."""
        self.encoder.set_controller_state(controller_tokens)
...
class BertLMHeadModelKVModulated(BertPreTrainedModel):
    ...
    def __init__(self, config, kv_config: Optional[Dict] = None):
        ...
        self.bert = BertModelKVModulated(
            config,
    ...
    def set_controller_state(self, controller_tokens: Optional[torch.Tensor]):
        """Set controller tokens for all cross-attention layers."""
        self.bert.set_controller_state(controller_tokens)
