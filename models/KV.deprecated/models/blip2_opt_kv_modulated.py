...
class Blip2OPTKVModulated(Blip2Base):
    ...
    def forward(self, samples):
        ...
        prompt_strings = [f"Question: {q} Answer:" for q in questions]
        prompt_with_space = [f"{p} " for p in prompt_strings]

        if answers is not None:
            text_sequences = [
                f"{prompt} {a}".strip()
                for prompt, a in zip(prompt_strings, answers)
            ]
        else:
            text_sequences = prompt_with_space
    ...
    def predict_answers(
        ...
        **kwargs
        ...
        with self.maybe_autocast():
            ...
            prompt_strings = [f"Question: {q} Answer:" for q in questions]
            if prompt:
                text_input = [prompt.format(q) for q in questions]
            else:
                text_input = [f"{p} " for p in prompt_strings]
            ...
            generate_kwargs = {
                ...
                "min_new_tokens": max(min_len, 1),
                "eos_token_id": self.eos_token_id,
                "pad_token_id": pad_token_id,
                "length_penalty": length_penalty,
            }
            sequences = self.opt_model.generate(**generate_kwargs)

            prompt_lengths = opt_tokens.attention_mask.sum(dim=1)
            ...
            for seq, prompt_len in zip(sequences, prompt_lengths):
                ...
                if gen.numel() == 0:
                    gen = seq[-1:].clone()
                generated_tokens_per_sample.append(gen)

            output_text = [
                self.opt_tokenizer.decode(gen, skip_special_tokens=True).strip()
                for gen in generated_tokens_per_sample
            ]
